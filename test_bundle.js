...
function normalize(str) {
  ...
  if (![3, 6, 9, 12, 15, 18, 21, 24].includes(words.length))
    throw new Error("Invalid mnemonic");
  return { nfkd: norm, words };
}
function aentropy(ent) {
  abytes(ent);
  if (![4, 8, 12, 16, 20, 24, 28, 32].includes(ent.length))
    throw new Error("invalid entropy length");
}
function generateMnemonic(wordlist2, strength = 128) {
  anumber(strength);
  if (strength % 32 !== 0 || strength < 32 || strength > 256)
...
function ecdh(Point2, ecdhOpts = {}) {
  ...
  function randomSecretKey(seed2 = randomBytes_(lengths.seed)) {
    return mapHashToField(abytes(seed2, lengths.seed, "seed"), Fn2.ORDER);
  }
...
function ecdsa(Point2, hash, ecdsaOpts = {}) {
  ...
  function verify(signature, message, publicKey2, opts = {}) {
    ...
    try {
      ...
      const P = Point2.fromBytes(publicKey2);
...
var HDKey = class _HDKey {
  ...
  static fromMasterSeed(seed2, versions = BITCOIN_VERSIONS) {
    abytes(seed2);
    if (8 * seed2.length < 128 || 8 * seed2.length > 512) {
      throw new Error("HDKey: seed length must be between 128 and 512 bits; 256 bits is advised, got " + seed2.length);
    }
    const I = hmac(sha512, MASTER_SECRET, seed2);
...
// test.ts
window.bip39 = bip39_exports;
window.wordlist = wordlist;
var mn = generateMnemonic(wordlist, 32);
console.log("\u041C\u043D\u0435\u043C\u043E\u043D\u0438\u0447\u0435\u0441\u043A\u0430\u044F \u0444\u0440\u0430\u0437\u0430:", mn);
var isValid = validateMnemonic(mn, wordlist);
console.log("\u041C\u043D\u0435\u043C\u043E\u043D\u0438\u043A\u0430 \u0432\u0430\u043B\u0438\u0434\u043D\u0430:", isValid);
var seed = await mnemonicToSeed(mn, "");
console.log("Seed (hex):", Buffer.from(seed).toString("hex"));
var hdKey = HDKey.fromMasterSeed(seed);
var path = "m/44'/0'/0'/0/0";
var derivedKey = hdKey.derive(path);
var publicKey = derivedKey.publicKey;
...
var extendedPublicKey = derivedKey.publicExtendedKey;
...
async function verifyUser(mnemonic, storedPublicKey) {
  ...
  return publicKey2 === storedPublicKey;
}
export {
  extendedPublicKey,
  mn,
