...
import { getUserAllSessions, ServerWorkoutSession, clearUserWorkouts } from '../statsService';
...
export interface ProfileScreenProps {
    ...
    initialSessions?: SimpleWorkoutSession[];
    initialDate?: Date;
    initialSteps?: number;
    initialLoading?: boolean;
    // Callback overrides for testing
    onClearData?: () => void;
    onRefresh?: () => void;
}
...
export function ProfileScreen({
    ...
    const [allSessions, setAllSessions] = useState<SimpleWorkoutSession[]>(initialSessions);
    ...
    const [selectedDateSteps, setSelectedDateSteps] = useState<number>(initialSteps);
    ...
    const processSessionsData = (sessions: SimpleWorkoutSession[], steps: number) => {
        ...
        const displaySessions: SessionDisplay[] = sessions.map(session => {
            ...
            Object.entries(session.exercises || {}).forEach(([exerciseName, recordRaw]) => {
                const record = recordRaw as SimpleExerciseRecord;
                if (record && (record.duration > 0 || (record.reps && record.reps > 0))) {
                    exercises.push({
    ...
    const loadData = async () => {
        // If we are given initial sessions (like in Storybook), use those and skip network/storage
        if (initialSessions.length > 0) {
            processSessionsData(initialSessions, initialSteps);
            setLoading(false);
            return;
        }
        ...
        try {
            ...
            if (autoSyncEnabled) {
                try {
                    ...
                    if (user?.publicKey) {
                        ...
                        const convertedServerSessions: SimpleWorkoutSession[] = serverSessions
                            ...
                            .map((serverSession: ServerWorkoutSession) => {
                                const exercises =
                                    Object.values(EXERCISES).reduce((acc: Partial<Record<EXERCISES, SimpleExerciseRecord>>, ex) => {
                                        acc[ex as EXERCISES] = { duration: 0, reps: undefined };
                                        return acc;
                                    }, {}) as Record<EXERCISES, SimpleExerciseRecord>;

                                serverSession.exercise_sets.forEach((set: any) => {
                                    if (set.exercise_type === 'STEPS') return;
                                ...
                                return {
                                    ...
                                    exercises: exercises as Record<EXERCISES, SimpleExerciseRecord>,
                                };
                            });
                    ...
                    console.warn('[ProfileScreen] Failed to fetch from server', serverError);
                }
            }

            const steps = await getTodaySteps();
            processSessionsData(sessions, steps);
            ...
            [
                ...
                },
                {
                    ...
                    onPress: async () => {
                        try {
                            setLoading(true);
                            // Clear all local data related to workouts, steps, time bank, etc.
                            const keys = await AsyncStorage.getAllKeys();
                            const keysToRemove = keys.filter(key =>
                                key.startsWith('session_') ||
                                key.startsWith('@iSparta:steps_') ||
                                key === 'time_bank' ||
                                key === 'app_usage_history'
                            );
                            await AsyncStorage.multiRemove(keysToRemove);

                            // Clear server data
                            if (currentUserKey) {
                                await clearUserWorkouts(currentUserKey);
                            }
        ...
        <ScrollView style={styles.container}>
            ...
            <View style={styles.card}>
                ...
                {filterSessionsByDate(allSessions, selectedDate).length === 0 ? (
                    ...
                    filterSessionsByDate(allSessions, selectedDate).map((session: SimpleWorkoutSession, index: number) => (
                        <TouchableOpacity
                        ...
                        >
                            ...
                            {Object.entries(session.exercises || {}).map(([exerciseName, recordRaw]) => {
                                const record = recordRaw as SimpleExerciseRecord;
                                if (!record || (!record.duration && !record.reps && !record.kilometers)) return null;
